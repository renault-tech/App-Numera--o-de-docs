...
| 3.19 | ✅ 30/07/2026 — **Data de envio do documento** | Campo opcional na reserva (`reservations.sent_at`, migração 0010); puramente informativo, sem aviso de urgência nem sinalização de pendência em nenhuma tela — só um registro à disposição do usuário, editável depois pelo Histórico (exclusivo do autor, migração 0005). Incluído na exportação | 🟢 |
...
| 3.21 | ✅ 30/07/2026 — **Múltiplas secretarias de destino** | A secretaria de destino virou uma lista de caixinhas: um mesmo documento pode ser endereçado a várias secretarias, ou a "Todas as secretarias" (valor sentinela, que desmarca e desabilita as demais). `reservations.dest_secretarias jsonb` é a fonte da verdade e `dest_secretaria` segue preenchido com o rótulo montado para compatibilidade (migração 0011, doc 03 §1.4). `showConfirmDialog` ganhou o tipo de campo `checks` (seleção múltipla), usado também na edição da reserva | 🟢 |
| 3.22 | ✅ 07/08/2026 — **Correção: editar "Número inicial" de um tipo existente não mudava a próxima reserva** | Bug de produção (Decreto): o campo só gravava `documents.start_number`, legado após a criação do bucket (doc 03 §1.1) — sem efeito no `document_counters` real. Documentos sem numeração por secretaria não tinham nenhuma tela para corrigir isso depois do primeiro uso. `saveDoc()` agora propaga a mudança via `set_secretaria_counter` para esses documentos; rótulo do campo vira "Próximo número" ao editar, com aviso claro se o valor pedido já foi usado (RN-01). Só frontend, sem migração nova (doc 03 §1.1) | 🔴 |
| 3.23 | ✅ 07/08/2026 — **Correção: duplo toque na reserva gerava dois números para o mesmo pedido** | Bug de produção (Mem. 632/2026 e 633/2026, 275ms de diferença, mesmo assunto/destinatário/usuário): `confirmReserve()` não bloqueava o botão "Confirmar reserva" durante a chamada ao RPC, então um duplo toque disparava a função duas vezes — `reserve_number()` é atômico entre usuários diferentes (migração 0010) mas não é idempotente, então cada toque gerou uma reserva legítima e distinta. Corrigido desabilitando o botão e ignorando toques repetidos enquanto a reserva está em andamento (mesmo padrão de `handleLogin()`/cadastro). Doc 03 §3 | 🔴 |
